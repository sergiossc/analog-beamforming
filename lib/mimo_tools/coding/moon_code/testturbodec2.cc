...
int main()
{
   ...
   Turbodec decoder(p,gnum,gden,N,1,interleaveseed,P,2);
   ...
   unsigned char *encout;		// encoder output
   ...
   numit = 1;  double SNRlist[] = {2,3,4.5};
//   numit = 2;  double SNRlist[] = {0.25,0.5,1,1.5,2,2.5};
//   numit = 3;  double SNRlist[] = {0.25,0.5,0.75,1,1.5,1.75};
   ...
   for(int isnr = 0; isnr < nsnr; isnr++) {
	  ...
	  cout << "SNR(dB)=" << SNRdb << "  SNR=" << SNR << "   sigma2=" 
		   << sigma2 << endl;
	  decoder.sigma2 = sigma2;	//  set the noise variance
	  sigma = sqrt(sigma2);
	  blockcount = 0;
	  do {
...
/*
Local Variables:
compile-command: "g++ -o testturbodec2 -g testturbodec2.cc BCJR.cc BinConvIIR.cc interleave.cc Turboenc.cc Turbodec.cc gran.cc uran.cc"
End:
*/
